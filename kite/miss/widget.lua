...
function M.text(t)
	...
	for _,id in utf8.codes(text) do
		...
		if border then
			...
			table.insert(border_sprites, sprite2d.create(font.texture.id, x1-bz, y1, x1-bz, y3, x3-bz, y3, x3-bz, y1, table.unpack(c.texcoord)))
			table.insert(border_sprites, sprite2d.create(font.texture.id, x1+bz, y1, x1+bz, y3, x3+bz, y3, x3+bz, y1, table.unpack(c.texcoord)))
			table.insert(border_sprites, sprite2d.create(font.texture.id, x1, y1-bz, x1, y3-bz, x3, y3-bz, x3, y1-bz, table.unpack(c.texcoord)))
			table.insert(border_sprites, sprite2d.create(font.texture.id, x1, y1+bz, x1, y3+bz, x3, y3+bz, x3, y1+bz, table.unpack(c.texcoord)))
		end
	...
	function self.draw()
		if border then
			program.set_color(border_color)
